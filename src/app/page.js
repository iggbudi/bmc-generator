...
${bmcData.revenueStreams.map((item, i) => `${i + 1}. ${item}`).join('\n')}`;
  ...
  const downloadPDF = () => {
    ...
    // Title
    doc.setFontSize(16);
    doc.text(l.title, pageWidth / 2, yPosition, { align: 'center' });
    yPosition += 10;

    doc.setFontSize(10);
    doc.text(l.subtitle, pageWidth / 2, yPosition, { align: 'center' });
    yPosition += 8;

    // Business Idea
    doc.setFontSize(12);
    doc.setFont(undefined, 'bold');
    doc.text(l.businessIdea, margin, yPosition);
    yPosition += 6;
    doc.setFont(undefined, 'normal');
    doc.setFontSize(10);
    const ideaLines = doc.splitTextToSize(businessIdea, maxWidth);
    doc.text(ideaLines, margin, yPosition);
    yPosition += ideaLines.length * lineHeight + 5;

    // BMC Sections
    const sections = [
      { title: l.keyPartners, items: bmcData.keyPartners },
      { title: l.keyActivities, items: bmcData.keyActivities },
      { title: l.keyResources, items: bmcData.keyResources },
      { title: l.valuePropositions, items: bmcData.valuePropositions },
      { title: l.customerRelationships, items: bmcData.customerRelationships },
      { title: l.channels, items: bmcData.channels },
      { title: l.customerSegments, items: bmcData.customerSegments },
      { title: l.costStructure, items: bmcData.costStructure },
      { title: l.revenueStreams, items: bmcData.revenueStreams },
    ];
    ...
    sections.forEach((section) => {
      ...
      section.items.forEach((item, index) => {
        ...
        yPosition += itemLines.length * lineHeight;
      });
      yPosition += 3;
    });
